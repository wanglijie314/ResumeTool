...
- M5 AI 字段识别底座 ✅（建议级口径：AI 永不自动作答，接受才生效）
  ...
  - 简历解析补全额外打印流程日志（与通道请求同会话）：开始(文件名/遗漏行数/已解析字段数) → 完成(新增/修正/低把握忽略+耗时) / 失败(错误码，退回规则结果)
- M6 简历解析 AI 兜底 ✅（skill `resume-extract`）
  ...
  - 触发口径：AI 设置填了 Key → **每次导入简历都自动调用** AI 补全（发送遗漏片段+简历上下文≤2 万字给模型商，不弹确认；结果为可编辑草稿）；无 Key 完全退化为纯规则解析，不发送任何内容；任何失败退回规则结果。请求超时：默认/页面字段识别 120s、简历补全 180s（先提时实验排查网络异常）
